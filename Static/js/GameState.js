...
function renderOpponents() {
    ...
    opponents.forEach((opponent, index) => {
        ...
        if (Number(opponent.user_id) === Number(gameState.current_player)) {
...
function isMyTurn() {
    ...
    return Number(gameState.current_player) === getCurrentUserId();
}

function getCurrentTurnPlayer() {
    return (gameState?.players || []).find(
        (player) => Number(player.user_id) === Number(gameState.current_player)
    ) || null;
}
